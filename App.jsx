...
function GameCanvas({ phase, multiplierRef, startTime }) {
  ...
  useEffect(() => {
    ...
    function multToY(m, oy, availH) {
      const p = Math.min(Math.pow(Math.max(m - 1, 0), 0.38) / 2.1, 0.70)
    ...
    const render = () => {
      ...
      ctx.beginPath(); ctx.moveTo(ox-6, oy); ctx.lineTo(W-8, oy); ctx.stroke()
      ctx.strokeStyle = 'rgba(255,255,255,0.07)'; ctx.lineWidth = 1; ctx.setLineDash([3,10])
      ctx.beginPath(); ctx.moveTo(ox, oy); ctx.lineTo(ox, 16); ctx.stroke()
      ctx.setLineDash([])
      ...
      if (phase === 'flying') {
        ...
        // Draw curve — returns tangent angle at tip
        const tangentAngle = drawCurve(ox, oy, tx, ty)

        // ── Plane angle: Refined rotation for aerodynamic flight ──
        // Scale the angle down to keep the flight path looking aerodynamic
        // and clamp it to avoid "rocket-like" vertical takeoffs.
        let planeAngle = tangentAngle * 0.6
        planeAngle = Math.max(-0.43, Math.min(0.20, planeAngle))

        // Tiny breath wobble — very subtle, period ~3s
